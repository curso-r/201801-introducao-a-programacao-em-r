...
cyl_asfactor = as.factor(mpg$cyl)
class(cyl_asfactor)
table(cyl_asfactor)
...
Gŕafico de dispersão de displ por hwy, filtrando pela classe de carros pequenos. (compact, midsize, subcompact)
...
1. Observe a relação entre a arrecadação(gross) e orçamento(budget) dos filmes.

```{r}

```

2. Filtrando apenas por filmes norte-americanos, observe se a relação entre arrecadação e orçamento mudou ao longo dos anos.
...
Usando a base 'dados', faça um gráfico do `imb_score` por ano, apenas para o diretor Steven Spielberg, e "ligue os pontos".
...
dados %>% count(country, sort = T)


# fct_other
dados %>%
  mutate(country = fct_other(country, keep = 
                               c('USA', 'UK', 'France', 'Canada', 'Germany', 
                                 'Australia', 'India', 'Spain', 'China', 'Italy'), 
                             other_level = 'Outros')) %>% 
  na.omit() %>% 
  ggplot() +
  geom_bar(mapping = aes(x = country))
...
dados %>%
  na.omit() %>% 
  mutate(content_rating = fct_collapse(content_rating, Outros = 
                                         c("Approved", "G", "GP", "Passed", 
                                           "TV-14", "TV-G", "TV-MA", "TV-PG", "TV-Y7", "X"))) %>%
  ggplot() +
  geom_bar(mapping = aes(x = content_rating))
```
...
Comparar as notas `imb_score` do Tarantino, Scorsese e Tim Burton ao longo do tempo.
...
dados %>% 
  ...
  ggplot() +
  geom_point(aes(x = title_year, y = imdb_score, color = director_name))
```
